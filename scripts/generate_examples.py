...
from finance_requirements_generator.readiness_engine import (  # noqa: E402
...
def generate_examples(output_dir: Path) -> list[Path]:
    ...
    for process_key, intake in DEFAULT_SAMPLE_INPUTS.items():
        ...
        if process_key in READINESS_SAMPLE_PROCESS_KEYS:
            readiness_pack = generate_implementation_readiness_pack(pack)
            generated.append(
                export_readiness_markdown(
                    readiness_pack,
                    output_dir / f"{process_key}_implementation_readiness_pack.md",
                )
            )
            generated.append(
                export_readiness_docx(
                    readiness_pack,
                    output_dir / f"{process_key}_implementation_readiness_pack.docx",
                )
            )
