...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }}
    ...
    .flow-track {{ position:relative; width:1652px; height:300px; }}
    .flow-node {{
      position:absolute; z-index:2; top:22px; display:flex; width:180px; height:124px;
      flex-direction:column;
      align-items:center; justify-content:center; padding:12px 13px; border:2px solid var(--step);
      border-radius:6px; background:#fff; color:var(--ink); text-align:center;
    }}
    .flow-node.gate {{
      width:158px; border-color:var(--gate); background:var(--gate); color:#fff;
    }}
    .flow-node.decide {{
      width:170px; border-color:var(--decide); background:#fef3c7;
      color:#7c2d12;
    }}
    .flow-node.fix {{
      width:178px; height:88px; border-color:var(--fix);
      background:#fee2e2; color:#7f1d1d;
    }}
    .start-node {{ left:0; }}
    .source-node {{ left:200px; }}
    .validation-node {{ left:422px; }}
    .decision-node {{ left:644px; }}
    .approval-node {{ left:1050px; }}
    .reporting-node {{ left:1272px; }}
    .signoff-node {{ left:1494px; }}
    .exception-node {{ top:190px; left:840px; }}
    .node-kind {{
    ...
    .flow-connectors {{
      position:absolute; z-index:1; inset:0; width:1652px; height:300px; overflow:visible;
    }}
    ...
    .branch-label.no {{ top:63px; left:911px; }}
    .branch-label.yes {{ top:174px; left:739px; }}
    .legend {{
    ...
    <section class="card" aria-labelledby="flow-heading">
      ...
      <div class="flow-scroll">
        <div class="flow-track" role="img" aria-label="{escape(title)}">
          {_flow_connectors()}
          {_node(flow.trigger, "gate start-node", "Start")}
          {_node(flow.source, "step source-node", "Intake")}
          {_node(flow.validation, "step validation-node", "Validate")}
          {_node(flow.decision, "decide decision-node", "Decision")}
          {_node(flow.exception_resolution, "fix exception-node", "Remediate")}
          {_node(flow.approval, "step approval-node", "Approve")}
          {_node(flow.reporting, "step reporting-node", "Evidence")}
          {_node(flow.signoff, "gate signoff-node", "Sign-off")}
          <span class="branch-label no">No &middot; continue</span>
          <span class="branch-label yes">Yes &middot; remediate</span>
        </div>
      </div>
      {_legend()}
    </section>
...
def _flow_connectors() -> str:
    return """<svg class="flow-connectors" viewBox="0 0 1652 300" aria-hidden="true">
            <defs>
            ...
            <path d="M 158 84 H 200" marker-end="url(#flow-arrow)"></path>
            <path d="M 380 84 H 422" marker-end="url(#flow-arrow)"></path>
            <path d="M 602 84 H 644" marker-end="url(#flow-arrow)"></path>
            <path d="M 814 84 H 1050" marker-end="url(#flow-arrow)"></path>
            <path d="M 1230 84 H 1272" marker-end="url(#flow-arrow)"></path>
            <path d="M 1452 84 H 1494" marker-end="url(#flow-arrow)"></path>
            <path d="M 729 146 V 234 H 840" marker-end="url(#flow-arrow)"></path>
            <path d="M 1018 234 H 1140 V 146" marker-end="url(#flow-arrow)"></path>
          </svg>"""
